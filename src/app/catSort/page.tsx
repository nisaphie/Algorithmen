...
export default function Home() {
	...
	const [sortingAlgorithm, setSortingAlgorithm] = useState<"bubble" | "insertion">();
	...
	function handleSort() {
		...
		const bubbleSort = async () => {
			for (let i = 0; i < length - 1; i++) {
				for (let j = 0; j < length - i - 1; j++) {
					if (
						elements[j].props["data-key"] >
						elements[j + 1].props["data-key"]
					) {
		...
		const insertionSort = async () => {
			...
			setImageElements([...elements].sort((a, b) => a.props["data-key"] - b.props["data-key"]));
			for (let i=1; i<length; i++) {
				let key = elements[i];
				let j = i-1;
				while (j>=0 && elements[j].props["data-key"] > key.props["data-key"]) {
			...
			<div className="flex flex-col gap-4 p-4">
				<Card>
					<CardHeader className="flex flex-col">
						...
						<CardDescription>
							Hier kannst du ein Bild durchmischen und dann
							schrittweise sortieren lassen.
						</CardDescription>
					</CardHeader>
					<CardContent className="flex flex-col flex-grow gap-4 items-center">
						...
						<div className="flex flex-row gap-4 w-full">
							...
							<Select onValueChange={(value) => setSortingAlgorithm(value as "bubble" | "insertion")}>

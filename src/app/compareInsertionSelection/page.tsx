...
import { Cat, Equal, Loader2 } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function Home() {
  ...
  const [sortingAlgorithmLeft, setSortingAlgorithmLeft] =
    useState<String | null>();
  const [sortingAlgorithmRight, setSortingAlgorithmRight] =
    useState<String | null>();
  ...
  const [imageElementsLeft, setImageElementsLeft] = useState<
    React.ReactNode[] | null
  >();
  const [imageElementsRight, setImageElementsRight] = useState<
    React.ReactNode[] | null
  >();
  ...
  useEffect(() => {
    ...
    setImageElementsLeft(elements);
    setImageElementsRight(elements);
  }, [rows, columns]);
  ...
  function handleSort() {
    if (!sortingAlgorithmLeft)
      return alert("Please select a sorting algorithm");
    if (!sortingAlgorithmRight)
      return alert("Please select a sorting algorithm");
  ...
  function handleSortLeft() {
    ...
    const elements = imageElementsLeft as React.ReactElement<any>[];
    ...
    const selectionSort = async () => {
      ...
      for (let i = 0; i < n - 1; i++) {
        ...
        for (let j = i + 1; j < n; j++) {
          if (
            elements[j].props["data-key"] < elements[min_idx].props["data-key"]
          ) {
            min_idx = j;
          }
        }
    ...
    if (sortingAlgorithmLeft === "insertion") insertionSort();
    if (sortingAlgorithmLeft === "bubble") bubbleSort();
    if (sortingAlgorithmLeft === "selection") selectionSort();
  }

  function handleSortRight() {
    setImageState("loading");
    const elements = imageElementsRight as React.ReactElement<any>[];
    const length = elements?.length;
    if (!length) return;
    const bubbleSort = async () => {
      for (let i = 0; i < length - 1; i++) {
    ...
    if (sortingAlgorithmRight === "insertion") insertionSort();
    if (sortingAlgorithmRight === "bubble") bubbleSort();
    if (sortingAlgorithmRight === "selection") selectionSort();
  }
        ...
        <Card>
          <CardHeader className="flex flex-col">
            <CardTitle className="flex flex-row gap-1">
              <Equal size={16} /> Comparison
            </CardTitle>
            <CardDescription>
              Hier kannst du ein Bild durchmischen und sortieren und siehst
              InsertionSort und SelectionSort im Vergleich. Tipp: Besonders gut
              kommt der Vergleich zur Geltung wenn ein Slider (Zeilen oder
              Spalten) auf 1 gesetzt wird.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col flex-grow gap-4 items-center">
            ...
            <div className="flex flex-row gap-4 w-full">
              <Select onValueChange={(value) => setSortingAlgorithmLeft(value)}>
              ...
              <Select
                onValueChange={(value) => setSortingAlgorithmRight(value)}
              >
                <SelectTrigger className="w-[280px]">
                  <SelectValue placeholder="Algorithmus auswählen" />
                </SelectTrigger>
            ...
            <div className="flex flex-row gap-4 w-full">
              ...
              <Slider
                ...
                onValueChange={(value: number[]) => setSortingSpeed(value[0])}
              />
            </div>
          </CardContent>
        </Card>
